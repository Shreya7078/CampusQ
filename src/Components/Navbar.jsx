    ...
    <>
      ...
      <nav className="bg-gradient-to-r from-indigo-900 to-purple-600 p-4 flex justify-between items-center fixed w-full top-0 z-10 shadow-lg">
        ...
        <div className="hidden md:flex space-x-6 text-md">
          <Link
            to="/"
            className={`text-white hover:text-indigo-200 relative group ${location.pathname === '/' ? 'border-b-2 border-white' : ''}`}
          >
            Home
          ...
          <Link
            to="/dashboard"
            className={`text-white hover:text-indigo-200 relative group ${location.pathname === '/dashboard' ? 'border-b-2 border-white' : ''}`}
          >
            Dashboard
            <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-indigo-200 transition-all duration-300 group-hover:w-full"></span>
          </Link>
          <Link
            to="/support"
            className={`text-white hover:text-indigo-200 relative group ${location.pathname === '/support' ? 'border-b-2 border-white' : ''}`}
          >
            Support
            <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-indigo-200 transition-all duration-300 group-hover:w-full"></span>
          </Link>
          <Link
            to="/announcements"
            className={`text-white hover:text-indigo-200 relative group ${location.pathname === '/announcements' ? 'border-b-2 border-white' : ''}`}
          >
            Announcements
            <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-indigo-200 transition-all duration-300 group-hover:w-full"></span>
          </Link>
          <Link
            to="/profile"
            className={`text-white hover:text-indigo-200 relative group ${location.pathname === '/profile' ? 'border-b-2 border-white' : ''}`}
          >
            Profile
            <span className="absolute left-0 bottom-0 w-0 h-0.5 bg-indigo-200 transition-all duration-300 group-hover:w-full"></span>
          </Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-gradient-to-r from-indigo-900 to-purple-600 text-white p-4 absolute w-full mt-12 z-20">
          ...
          <Link
            to="/dashboard"
            className={`block py-2 ${location.pathname === '/dashboard' ? 'border-b-2 border-white' : 'hover:bg-indigo-600'}`}
            onClick={() => setIsMenuOpen(false)}
          >
            Dashboard
          </Link>
          <Link
            to="/support"
            className={`block py-2 ${location.pathname === '/support' ? 'border-b-2 border-white' : 'hover:bg-indigo-600'}`}
            onClick={() => setIsMenuOpen(false)}
          >
            Support
          </Link>
          <Link
            to="/announcements"
            className={`block py-2 ${location.pathname === '/announcements' ? 'border-b-2 border-white' : 'hover:bg-indigo-600'}`}
            onClick={() => setIsMenuOpen(false)}
          >
            Announcements
          </Link>
          <Link
            to="/profile"
            className={`block py-2 ${location.pathname === '/profile' ? 'border-b-2 border-white' : 'hover:bg-indigo-600'}`}
            onClick={() => setIsMenuOpen(false)}
          >
            Profile
          </Link>
